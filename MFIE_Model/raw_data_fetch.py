...
class RawDataFetcher(FactorGenerater):
    ...
    def _get_month_end(self, date):# 如果传入的 date 已是当月最后一天，就直接返回该日期；否则返回下一个月末的日期
        import calendar  ##日期操作
        import pandas.tseries.offsets as toffsets
        _, days = calendar.monthrange(date.year, date.month)
        if date.day == days:
            return date
        else:
            return date + toffsets.MonthEnd(n=0) ##修改1  原为date + toffsets.MonthEnd(n=1)

    @retry(stop_max_attempt_number=500, wait_random_min=1000, wait_random_max=2000)  ##如果失败，最多重试500次
    def ensure_data(self, func, save_dir, start_dt='20010101', end_dt='20201231'):
        """ 确保按交易日获取数据，下载date缺少的股票数据
        """
        ...
        for tday in tdays:
            ...
            datdf.to_csv(path, encoding='gb18030')
    ...
    def ensure_data_by_q(self, func, save_dir,start_dt='20010101', end_dt='20201231'):
        ...
        dl = sorted(dl)
        if len(dl) > 3:
            dl = dl[0:len(dl)-3] #已经存在的最后三个季度数据重新下载--部分公司会修正数据
        s = pd.to_datetime(start_dt)
        e = pd.to_datetime(end_dt)
        ...
        datdf = []
        # year = tday.year
        # quarter = (tday.month-1)//3 + 1
        df = pd.read_csv("/home/linyuchang/wcp/model/raw_data/src/all_stocks.csv",encoding="gb18030")
        stock_list = sorted(df["code"].drop_duplicates().tolist())
        for ts_code in stock_list:
            data_code = func(ts_code=ts_code)
            print(f"{ts_code}已完成,数据共有{len(data_code)}")
            if data_code is  not None and len(data_code)>0:
                datdf.append(data_code)
            time.sleep(1)
        print("读取结束开始合成")
        datdf = pd.concat(datdf)
        datdf["end_date"] = datdf["end_date"].astype(str).str.strip()
        datdf = datdf.sort_values("end_date")
        datdf = datdf.set_index("end_date",drop=False)
        for tday in qdates:
            if tday in dl:continue
            t = tday.strftime("%Y%m%d")
            try:
                data = datdf.loc[[t]]
                path = os.path.join(tmp_dir, t+".csv")
                data.reset_index(drop=True).to_csv(path, encoding='gb18030')
                print(t+".csv write ok !!!!!")
            except:
                pass

    def create_indicator(self, raw_data_dir, raw_data_field, indicator_name):
        ''' 主要用于通过日频数据创建日频指标--把分散在多个 “单日数据文件” 里的某类原始字段（比如收盘价、成交量），整合为一张「全市场股票 × 所有交易日」的指标宽表
        '''
        ...
        for f in os.listdir(tmp_dir):
            ...
            dat = pd.read_csv(os.path.join(tmp_dir, f), index_col=['ts_code'], engine='python', encoding='gb18030')
        ...
        self.close_file(df, indicator_name)  ##保存文件
    ...
    def create_indicator_m_by_d(self, raw_data_dir, raw_data_field, indicator_name, start_dt='20010101', end_dt='20201231'):
        ...
        s = pd.to_datetime(start_dt)
        e = pd.to_datetime(end_dt)
        new_tdays = self._get_trade_days(s, e, "M")  ##每月最后交易日
        new_caldays = [self._get_month_end(tdate) for tdate in new_tdays]  ##每月最后自然日
        ...
        for tday in new_tdays:
            ...
            dat = pd.read_csv(os.path.join(tmp_dir, name+".csv"), index_col=['ts_code'], engine='python', encoding='gb18030')
    ...
    def create_indicator_m_by_d_ex(self, raw_data_dir, raw_data_field, indicator_name, start_dt='20010101', end_dt='20201231'):
        ...
        datdf = getattr(self, indicator_name, None) #从内存读取
        datdf = self.preprocess(datdf)#数据预处理
        self.close_file(datdf, indicator_name)
        #
        s = pd.to_datetime(start_dt)
        e = pd.to_datetime(end_dt)
        new_tdays = self._get_trade_days(s, e, "M")
        new_caldays = [self._get_month_end(tdate) for tdate in new_tdays] ##自然月末--批量查询
        all_stocks_info = self.meta
        df = pd.DataFrame(index=all_stocks_info.index, columns=new_caldays)
        for tday in new_tdays:
            caldate = self.month_map[tday]##自然月末--查表高效
    ...
    def create_indicator_m_by_q(self, raw_data_dir, raw_data_field, indicator_name, start_dt='20010101', end_dt='20201231'):
        ...
        for d in qdays: #每季度最后一天
            ...
            dat = pd.read_csv(os.path.join(tmp_dir, name+".csv"), index_col=['ts_code'], engine='python', encoding='gb18030', parse_dates=['ann_date','end_date'])
            ...
            del dat['Unnamed: 0']
            panel[d] = dat
        datpanel = pd.concat(panel, axis=1, keys=panel.keys())
        datpanel = datpanel.stack(level=0)
        ...
        for d in df.columns: #每月最后一天
            for stock in df.index: #每只股票
                try:
                    datdf = datpanel.xs(stock,level=0,axis=0)
    ...
    def create_indicator_m_by_q_ex(self, raw_data_dir, raw_data_field, indicator_name, start_dt='20010101', end_dt='20201231'):
        ...
        for d in qdays: #每季度最后一天
            ...
            diff = dat.index.difference(all_stocks_info.index) #删除没在股票基础列表中多余的股票行
            dat = dat.drop(labels=diff)
            dat = dat[~dat.index.duplicated(keep='last')] #财务数据中同一只股票可能会有重复的记录,删除多余重复的 --非重复--False 重复--True
            del dat['Unnamed: 0']
            panel[d] = dat
          #  print(d)
        datpanel = pd.concat(panel, axis=1, keys=panel.keys())
    ...
    def create_daily_quote_indicators(self):
        ...
        for name in benchmarks:
            dat = pd.read_csv(os.path.join(tmp_dir, name+".csv"), index_col=[2], engine='python', encoding='gb18030', parse_dates=['trade_date'])
            pct_chg.loc[name] = dat['pct_chg']
            close.loc[name] = dat['close']
            hfq_close.loc[name] = dat['close']
        #更新数据
        pct_chg = pct_chg / 100
        self.close_file(pct_chg, 'pct_chg')
        self.close_file(close, 'close')
        ...
        for m_end_date in months_end:
            ...
            m_end_date = pd.to_datetime(m_end_date).strftime("%Y-%m-%d")
            m_start_date = pd.to_datetime(m_start_date).strftime("%Y-%m-%d")
            # try:
            pct_chg_M[self.month_map.loc[m_end_date]] = hfq_close.loc[:,m_end_date] / hfq_close.loc[:,m_start_date] - 1
            # except:
            #     print(f"{m_end_date,hfq_close.index[0]}数据为空")
            #     pass
            ...
            if period != 1: 
                for m_end_date in months_end[::-1]:
                    try:
                        ...
                        pct_chg_Nm[self.month_map[m_end_date]] = s##1-3月收益率记录到3月
                    except KeyError:
                        print(m_end_date)
                        break
            else:
    ...
    def preprocess(self, datdf, suspend_days_process=False, val=np.nan): #Suspend_days_process=False 直译是 “停牌日处理 = False”
        ''' 数据预处理
        '''
        datdf = datdf.copy()
...
class TushareFetcher(RawDataFetcher):
    ...
    def industry_citic(self):
        ...
        df.to_csv("/home/linyuchang/wcp/model/raw_data/src/industry_citic")
    ...
    def fetch_trade_day(self):
        """ 交易日列表
        """
        df = self.pro.trade_cal(is_open='1')##只返回开盘日
    ...
    def fetch_month_map(self):
        ...
        new_dates = s_dates.resample('M').apply(func_last)##重采样会自动生成日历月末
    ...
    #日数据
    def daily(self, t):
        return self.pro.daily(trade_date=t) ##返回指定时间t的行情数据 

    def suspend_d(self, t):
        return self.pro.suspend_d(trade_date=t)#停牌复牌数据

    def limit_list(self, t):
        return self.pro.limit_list(trade_date=t)##涨跌停数据

    def adj_factor(self, t):
        return self.pro.adj_factor(trade_date=t)#复权因子数据

    def daily_basic(self, t):
        return self.pro.daily_basic(trade_date=t)#每日基本面指标

    def moneyflow(self, t):
        return self.pro.moneyflow(trade_date=t)##资金流向指标
    #------------------------------------------------------------------------------------

    def segment_op(limit, _max):##装饰器工厂函数
        """ 分段获取数据
        """
        def segment_op_(f):##生成的装饰器
            #
            @wraps(f)
            def wrapper(*args, **kwargs):
                dfs = []
                for i in range(0, _max, limit):
                    kwargs['offset'] = i##拉取数据的起始位置
                    df = f(*args, **kwargs)
                    if len(df) < limit:
                        if len(df) > 0:
                            if len(df)>0:
    ...
    #------------------------------------------------------------------------------------
    #季度数据
    @segment_op(limit=100, _max=100000)
    def fina_indicator(self, *args, **kwargs):
        ...
        return self.pro.fina_indicator(*args, **kwargs)##vip

    @segment_op(limit=100, _max=100000)
    def income(self, *args, **kwargs):
        return self.pro.income(*args, **kwargs)  ##vip

    @segment_op(limit=100, _max=100000)
    def balancesheet(self, *args, **kwargs):
        return self.pro.balancesheet(*args, **kwargs)##vip
    
    @segment_op(limit=100, _max=100000)
    def cashflow(self, *args, **kwargs):
        return self.pro.cashflow(*args, **kwargs)##vip
    ...
    def index_daily(self):
        ...
        tmp_dir = os.path.join(self.root, "__temp_index_daily__")
        # data_list = pd.read_csv("/home/linyuchang/wcp/model/raw_data/src/month_map.csv",index_col=[0])
        # data_list.index = data_list.index.str.replace("-","")
        # data_list = [date for date in data_list.index if (date>="20000101") & (date<="20201231")]
        for i in index_list:
            df = self.pro.index_monthly(ts_code=i,fields='ts_code,trade_date,open,high,low,close,vol,amount')   
    ...
    def create_month_tdays_begin_end(self, latest_month_end_tradeday=None):## latest_month_end_tradeday最新月末交易日
        ''' 每月第一个和最后一个交易日映射
        '''
        tdays = self.tradedays
        ...
        if months_end[-1] > latest_month_end_tradeday:##防止超出期限，截断日期
    ...
    def create_trade_status(self):
        ...
        for f in os.listdir(tmp_dir):
            tday = pd.to_datetime(f.split(".")[0])
            dat = pd.read_csv(os.path.join(tmp_dir, f), index_col=[1], engine='python', encoding='gb18030')
            common_index = dat.index.intersection(df.index)
            df.loc[common_index, tday] = 0 #停牌的设置为0
           # print(tday)
    ...
    def create_maxupordown(self):
        ...
        for f in os.listdir(tmp_dir):
            ...
            #==================================================
            common_index = df.index.intersection(index)
            df.loc[common_index, tday] = 1 #涨跌停的设置为1
            #print(tday)
    ...
    def create_mkt_cap_float_m(self):
        ...
        for tday in new_tdays:
            name = tday.strftime("%Y%m%d")
            dat = pd.read_csv(os.path.join(tmp_dir, name+".csv"), index_col=[1], engine='python', encoding='gb18030')
            caldate = self.month_map[tday]
    ...
    def create_pcf_ncf_ttm_m(self):
        ...
        for qday in qdays:
            ...
            dat = pd.read_csv(os.path.join(tmp_dir, name+".csv"), index_col=[1], engine='python', encoding='gb18030', parse_dates=['ann_date'])
    ...
    def create_pcf_ocf_ttm_m(self): ##没有补缺失值的操作，更稳健
        ...
        for d in qdays:
            name = d.strftime("%Y%m%d")
            dat = pd.read_csv(os.path.join(tmp_dir, name+".csv"), index_col=[1], engine='python', encoding='gb18030', parse_dates=['ann_date','end_date'])
            diff = dat.index.difference(all_stocks_info.index) #删除没在股票基础列表中多余的股票行
        ...
        panel = pd.concat(panel, axis=1, keys=panel.keys())
        panel = panel.stack(level=0)
        ...
        for calday in df_result.columns: #每月最后一天
            for stock in df_result.index: #每只股票
                try:
                    datdf = panel.xs(stock, level=0, axis=0)
                    day = calday.strftime("%Y%m%d")
                    datdf = datdf.loc[datdf['ann_date']<day] #在那个历史节点,只能使用已经发布的财报,防止使用未来数据
...
#     def __init__(self):
#         self.conn = pymysql.Connect(host='x', user='x', password='x', db='wind', charset='gb18030')
#         super().__init__(using_fetch=True)

#     def daily(self, t):
#         with self.conn.cursor(cursor=pymysql.cursors.DictCursor) as cursor:  ##该cursor能让数据以字典形式返回
