...
def get_factor_names():
    global work_dir
    factor_info = pd.read_csv(os.path.join(work_dir, '可用因子统计.csv'), encoding='utf-8')
...
def get_ind_mktcap_matrix(datdf, ind=True, mktcap=True):
    ...
    if mktcap:
        ...
        lncap = pd.DataFrame()
    if ind:
        ind_dummy_matrix = pd.get_dummies(datdf[f'industry_{industry_benchmark}']).astype(int) #转换为行业哑变量
    else:
...
def get_datdf_in_year(year):  ##读取指定年份的所有月度文件
    global factor_path
    dates = []
...
def test_yearly(factors=None, start_year=2012, end_year=2019):
    ...
    #存储检验结果表格
    test_result = pd.concat(test_result,axis=1,keys=test_result.keys())
    test_result = test_result.unstack(level=0)
    # test_result = test_result.swapaxes(2, 0)  ##多维交换
...
def bar_plot(ax, x, y, title):
    ...
    ax.bar(x, y)
    ax.xaxis.set_major_locator(ticker.MultipleLocator(tick_spacing1))  ##以固定间隔显示刻度
    ax.set_title(title)
...
def plot_layerdivision(records, fname, concise):
    ...
    """
    如果分母的索引匹配分子的列索引 → 分母按列方向广播（每列所有行 ÷ 该列对应的分母值）；
    如果分母的索引匹配分子的行索引 → 分母按行方向广播（每行所有列 ÷ 该行对应的分母值）；
    """
...
def bar_plot_yearly(records, fname, concise):  ##柱形图
    global sf_test_save_path, num_layers
...
def layer_division_backtest(factors):
    ...
    for fname in factors: #逐个因子进行回测
        ...
        facdat = pd.read_csv(os.path.join(factor_matrix_path, openname+'.csv'), encoding='utf-8', engine='python', index_col=[0])
