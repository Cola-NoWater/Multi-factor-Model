...
import pandas as pd
import sys
sys.path.append("/home/linyuchang/wcp/model/MFIE_Model")
...
if __name__ == '__main__':
    ...
    date = pd.to_datetime("2014-01-31")
    factor_generater.month_map[factor_generater.month_map==date].index[0]
    df = pd.read_csv("/home/linyuchang/wcp/model/raw_data/industry_citic.csv",index_col=[0])
    df.columns = pd.to_datetime(df.columns).strftime("%Y%m%d")
    df.to_csv("/home/linyuchang/wcp/model/raw_data/industry_citic.csv")
    df["BP"].isna().sum()
    df.iloc[:,5:]
    df.iloc[:,0].tolist()
