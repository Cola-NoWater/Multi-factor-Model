...
for in_path in [file_path,save_path,visu_path,quality_path,matrix_path,inscmp_path]:
...
def fill_na(data, ind='zx'):##行业中文数补充缺失值
    """
...
def plot_industry_comparison(factor_name, plot_data):
    ...
    plt.subplot(111)  #1*1的画布一个子图
    sns.barplot(x=plot_data.index, y=factor_name, data=plot_data) ##以行业为x
...
def factor_industry_comparison(panel_path, matrix_path, sub_dir_name):
    ...
    #读取行业分类
    industry_citic = pd.read_csv(os.path.join(matrix_path, 'industry_zx.csv'), engine='python', encoding='utf-8', index_col=[0]).filter(regex='^(?!Unnamed)')
    industry_citic.columns = pd.to_datetime(industry_citic.columns)
    for fac in facs:
        fac = fac.replace('/', '_div_')
        r_df = pd.DataFrame()
        datdf = pd.read_csv(os.path.join(matrix_path, f'{fac}.csv'), engine='python', encoding='utf-8', index_col=[0])
        ...
        r_df = r_df.mean(axis=1).to_frame(fac)  #按行（时间）求均值
