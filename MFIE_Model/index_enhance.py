...
def plot_corr_heatmap(corr, method, preprocessed=True):
    ...
    if not os.path.exists(corrfig_path):
        ...
        if os.path.exists(save_path):
            try:
                save_num = sorted(int(f) for f in os.listdir(save_path))[-1] + 1   ##1.修改
            except:
                save_num = 1
        else:
            os.mkdir(save_path)
    ...
    fig, ax = plt.subplots(1, 1) ##一行一列 单个绘图画布
    sns.heatmap(corr, 
            linewidths=0.05, 
            vmin=-1, 
            vmax=1, 
...
def concat_factors_panel(factors=None, factors_dict=None, codes=None, ind=True, mktcap=True):
    ...
    if (factors_dict is None) or ('MKT_CAP_FLOAT' in factors) or (f'industry_{industry_benchmark}' in factors):  ##行业和市值是基础列，存储路径和其他有所不同
        matrix = {}
        for fac in factors:
...
def factor_return_forecast(factors_x, factor_data=None, window=12, half_life=6):
    ...
    if half_life:
        ...
        factor_rets = factor_rets.rolling(window=window).apply(wt_sum, args=(exp_wt,))###   修改 删除滞后.shift(1)  #先计算结果，再shift(1)
    else:
...
def linear_programming(data_dict):
    ...
    industry_data = pd.read_csv("/home/linyuchang/wcp/model/raw_data/industry_citic.csv",index_col=[0])
    ret_60 = get_market_status()
    defensive_sectors = {
            '食品','饮料', '医药','生物', '公用', '电力',
            '高速','公路', '铁路', '通信', '环境', '农','林','牧','渔'
        }
    for date in est_stock_rets.columns:
        ...
        if ret_60.loc[date] == "bad":
            ...
            key = date_month_end.strftime("%Y%m%d")
            all_industry = industry_data[key].unique()
            all_industry = [ind for ind in all_industry if pd.notna(ind)]
            for ind in all_industry:
                is_def = any(k in str(ind) for k in defensive_sectors)
                sector_upper[ind] = 0.15 if is_def else 0.05         
            ind_series = industry_data.loc[benchmark_wt.index, key]
            ind_dummy = pd.get_dummies(ind_series, prefix="sector").astype(int)
            ind_dummy = ind_dummy.reindex(columns=["sector_"+s for s in all_industry], fill_value=0)
            #ind_dummy = pd.get_dummies(industry_data.loc[benchmark_wt.index,key],prefix="sector",categories=all_industry).astype(int)
            A_sector = ind_dummy.T
            b_sector = np.array([sector_upper[s] for s in all_industry])*1.0
        else:
            A_sector = None
            b_sector = None
...
def stratified_sample(data_dict):
    ...
    for date in sorted(data_panel.keys()):
        ...
        for name, df in panel.groupby('industry_zx'):##name对应分组的行业
            num = len(df) // 3
            remainder = len(df) % 3
            if len(df) <= 3:  #对每个行业，若股票数≤3 则直接取权重；若 > 3 则按流通市值（MKT_CAP_FLOAT）降序 分成 3 层；
...
def plot_industry_bar(save_path, tdate, plot_data, title):
    ...
    for i, (key, df) in zip(range(len(plot_data)), plot_data.items()):
        plt.subplot(int("12" + str(i+1))) #一行两列  plot_data长度恒为2
...
def stock_weights_analysis(save_path, stock_wt, index_wt):
    ...
    stock_wt = stock_wt.shift(1, axis=1) #每个单元存放的是下一期的权重--根据下期收益率算的下期权重
    ...
    for tdate in stock_wt.columns:
        ...
        name = str(tdate)[:10]
        df_stock_wt.to_csv(os.path.join(hs_save_path, f'{name}.csv'), encoding="utf-8")