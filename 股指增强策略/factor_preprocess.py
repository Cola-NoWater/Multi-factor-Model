...
info_cols = ['code', 'name', 'ipo_date',"industry",'industry_zx', 'industry_sw', 'MKT_CAP_FLOAT', 'is_open1', 'PCT_CHG_NM']

work_dir = os.path.join(os.path.dirname(__file__), '因子预处理模块')
...
industry_benchmark = '' #行业基准（用于缺失值填充和中性化）
                          #zx - 中信； sw - 申万
...
def fill_na(data, ind=''):##行业中位数补充缺失值
    """
    ...
    facs_to_fill = [fac for fac in facs_to_fill             #筛选缺失值少于10%的因子
                    if pd.isnull(datdf[fac]).sum() / len(datdf) <= 0.1] #True为1,False为0
    for fac in facs_to_fill:
        try:
            fac_median_by_ind = datdf[[f'industry{ind}', fac]].groupby(f'industry{ind}').median()
        except:
            #强制转换为数字类型
            datdf.loc[:, fac] = datdf[[fac]].apply(coerce_numeric)
            fac_median_by_ind = datdf[[f'industry{ind}', fac]].groupby(f'industry{ind}').median()
        fac_ind_map = fac_median_by_ind.to_dict()[fac]
        fac_to_fill = datdf.loc[pd.isnull(datdf[fac]), [f'industry{ind}', fac]] #找出所有为空的因子字段
        fac_to_fill.loc[:, fac] = fac_to_fill[f'industry{ind}'].map(fac_ind_map) #填充这些为空的因子字段
...
def winsorize(data, n=5):
    ...
    dm1 = np.nanmedian(np.abs(fac_vals - dm), axis=0)  ##差的中位数
...
def neutralize(data, ind='zx'):
    ...
    ind_dummy_matrix = pd.get_dummies(datdf[f'industry{ind}'])
    ...
    datdf.loc[residue.index, cols_neu] = residue.values
    return datdf
...
def standardize(data):
    ...
    ##修改 自行添加排除全空列
    col_list = []
    for col in datdf.columns.tolist():
        if all(pd.isnull(datdf[col])):
            col_list.append(col)
    facs_to_sta = datdf.columns.difference(set(info_cols+col_list))
    dat_sta = datdf[facs_to_sta].replace([np.inf, -np.inf], np.nan).values
...
#         plt.subplot(9,9,i+1)
#         sns.displot(datdf[factor].fillna(0).values)
#         plt.title(factor)
...
def factor_data_quality_check(factors_path, factor_names, save_path, sub_dir_name, usable_factor_stat=False):
    ...
    for f in l:
        ...
        data = pd.read_csv(os.path.join(factors_path, f), engine='python', encoding='utf-8', index_col=[0])
        ...
        print(dt)
    df_null_stat.to_csv(os.path.join(save_path, "缺失值统计.csv"), encoding='utf-8')
    df_zero_stat.to_csv(os.path.join(save_path, "零值统计.csv"), encoding='utf-8')
    if usable_factor_stat:
        r = reduce(lambda left,right:left|right, l_usable_factor_stat)  ##遍历列表，找到有缺失的因子
        r = r[~r]
        print(r)
        r.to_csv(os.path.join(save_path, "可用因子统计.csv"), encoding='utf-8')
...
def process_cross_section(fpath, factor_names=None, visualized=False):
    ...
    #读取原始因子截面数据
    data = pd.read_csv(os.path.join(file_path, fpath), engine='python', encoding='', index_col=[0])
    #根据输入的因子名称将原始因子截面数据分割
    ...
    data_final.to_csv(os.path.join(save_path, fpath), encoding='utf-8')
    print(f'{name}处理完毕')

def data_to_matrix(panel_path,matrix_path,name):
    matrix_path = os.path.join(matrix_path,name)
    if not os.path.exists(matrix_path):
        os.mkdir(matrix_path)
    f = (os.listdir(panel_path)[:])[0]
    data = pd.read_csv(os.path.join(panel_path, f), engine='python', encoding='utf-8', index_col=[0])
