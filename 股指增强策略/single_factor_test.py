...
__all__ = ['single_factor_test', 'get_factor_names', 'panel_to_matrix', 'Backtest_stock', 'regress']
...
factor_path = os.path.join(work_dir, '因子预处理模块')
quality_path = os.path.join(factor_path, '因子数据品质检验',"已预处理")
factor_path = os.path.join(factor_path, '因子（已预处理）')
...
def get_factor_names():
    global work_dir
    factor_info = pd.read_csv(os.path.join(quality_path, '可用因子统计.csv'), encoding='utf-8')
    return  factor_info.iloc[:,0].tolist()
...
def regress(y, X, w=1, intercept=False):
    ...
    ts, params = result.tvalues, result.params #t值,回归系数(因子收益) 回归系数和t值是根据x的变量数量决定
    ts.index = X.columns
...
def get_ind_mktcap_matrix(datdf, ind=True, mktcap=True):
    ...
    if mktcap:
        ...
        lncap = pd.DataFrame()
    if ind:
        ind_dummy_matrix = pd.get_dummies(datdf['industry']).astype(int) #转换为行业哑变量
    else:
...
def regression_summary(ts, frets, ics):
    ...
    ts = ts.dropna().values #t统计量序列
    frets = frets.dropna().values #因子收益序列
    ics = ics.dropna().values #ic系数序列
...
def t_ic_test(datpanel, factor_name):
    ...
    for date, datdf in datpanel.items():
        ...
        t_series[date] = ts[factor_name].item()
        fret_series[date] = f_rets[factor_name].item()
...
def get_datdf_in_all():  ##读取所有年份的月度文件
    global factor_path
    dates = []
    ...
    datpanel = {}
    for date in dates: #
        datdf = pd.read_csv(os.path.join(factor_path, date+'.csv'), engine='python', encoding='utf-8', index_col=[0])
...
def test_all(factors=None, start_year=2012, end_year=2019):
    
    global sf_test_save_path
    test_result = {}
    ts_all, frets_all, ics_all = pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
    datpanel = get_datdf_in_all()
    if factors is None:
    ...
    for save_name, df in zip(['t_value', 'factor_return', 'ic'], [ts_all, frets_all, ics_all]):
        df.to_csv(os.path.join(sf_test_save_path, save_name+'.csv'), encoding='utf-8')
    test_result.to_csv(os.path.join(sf_test_save_path, 'T检验&IC检验结果.csv'), encoding='utf-8')
...
class Backtest_stock:
    ...
    def __init__(self, *, market_data, start_date, end_date, benchmarkdata=None, 
                 ...
                 refreshdays=None, rf_rate=0.04, use_pctchg=False,market_status=None,**kwargs):
        ...
        self.factor_generater = FactorGenerater()
    ...
    def _get_date_idx(self, date):
        ...
        date = pd.to_datetime(date)
        try:
            idx = datelist.index(date)  ##传入的日期不是交易日，返回最近日期，优先前一个日期
        except ValueError:
    ...
    def after_backtest(self):  ##返回当前权重持仓
        ...
        if self.use_pctchg:##使用涨跌幅回测
            ...
            self.portfolio_record['benchmark_nv'] = bm/bm.iloc[0]
    ...
    def cal_weighted_pctchg(self, date):  ##计算净值
    ...
    def _get_excess_acc_ret(self, start_date=None, end_date=None):
        ...
        excess_ret = nv_ret.values.flatten() - bm_ret.values.flatten()  ##扁平化为一维数组
...
class SingleFactorLayerDivisionBacktest:
    ...
    def get_stock_weight(self, equal_weight=True):
        ...
        result = pd.Panel.from_dict(result)   ##Panel已经弃用，会导致报错
        result = [result.minor_xs(group) for group in result.minor_axis]  ##按照group维度提取数据
        return result
    ...
    def cal_weight(self, stock_pool):
        ...
        #调整尾差
        if len(weights[-1]) == 1:#分层时因为权重拆分，最后一层可能只留下「某只股票拆分后的极小尾差权重」（比如只有(1, 0.0001)这一个元素）；
...
def panel_to_matrix(factors, factor_path=factor_path, save_path=sf_test_save_path):
    ...
    if '已预处理' in factor_path:
        factors.extend(['PCT_CHG_NM', 'industry', 'MKT_CAP_FLOAT'])
    datpanel = {}
    for f in os.listdir(factor_path):
        open_name = f.replace('_div_', '/')
        datdf = pd.read_csv(os.path.join(factor_path, open_name), encoding='utf-8', index_col=['code'], engine='python')
        date = pd.to_datetime(f.split('.')[0])
        datpanel[date] = datdf[factors]
    datpanel = pd.concat(datpanel, axis=1, keys=datpanel.keys())
    ...
    for factor in datpanel.columns:
        ...
        dat.to_csv(os.path.join(factor_matrix_path, save_name+'.csv'), encoding='utf-8')
