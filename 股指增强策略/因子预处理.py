...
def main():
    ...
    input(f'比较完毕,请到 {quality_path} 查看结果. 按任意键继续...')
    #收集需要处理的因子名称
    while True:
    ...
    #并行
    function_list = [delayed(process_cross_section)(fpath, factor_names) for fpath in os.listdir(file_path)[:]]
    Parallel(n_jobs=10, backend='multiprocessing')(function_list)  #并行化处理Parallel(n_jobs=10, backend='multiprocessing')(function_list) 
    print('因子截面数据已全部处理！')
    ##截面转矩阵
    data_to_matrix(save_path, matrix_path, "因子矩阵")
    input(f'比较完毕,请到 {matrix_path} 查看结果. 按任意键继续...')
    # #
