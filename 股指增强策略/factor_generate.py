...
class Data:
    ...
    def close_file(self, df, name,**kwargs):
        if name == 'meta':
            df.to_csv(os.path.join(self.root, 'src', self.metafile), encoding='utf-8', **kwargs)
        elif name == 'month_map':
            ...
            df.to_csv(os.path.join(self.root, 'src', self.industryfile), encoding='gbk', **kwargs)
        else:
            path = self.freqmap.get(name, None)
            if path is None:
                path = self.root
...
class FactorGenerater:
    def __init__(self, using_fetch=False):
        self.data = Data()
        self.industry_citic = pd.read_csv(os.path.join(WORK_PATH,"industry_citic.csv"),index_col="code")
        self.hs300_wt = pd.read_csv(os.path.join(WORK_PATH,"src","hs300.csv"),index_col="con_code")
    ...
    def save_file(self, datdf, path):
        ...
        save_cond1 = (~datdf['name'].str.contains('ST'))
        save_cond2 = (~pd.isnull(datdf['industry_sw'])) & (~datdf['industry_sw'].str.contains('0'))
        save_cond3 = (~pd.isnull(datdf['MKT_CAP_FLOAT']))
    ...
    def get_basic_data(self, tdate):
        ...
        df0["PCT_CHG_NM"] = self.get_next_pctchg(stocklist, tdate)
        return stocklist, df0
    ...
    def get_last_month_end(self, date):
        ...
        return datetime(lstyear, lstmonth, 1) + toffsets.MonthEnd(1)
    ...
    def _get_regress_data(self, stocks, tdate, dates, params=("000001.SH", 60)):
        ...
        col_index = self._get_period(tdate, offset=-period, freq="M", datelist=dates, resample=False) 
    ...
    def _get_barra_quote_data(self, stocks, tdate):##行情类因子
